...
`docs/exec-plans/qb-replay-012-canonical-replay-time-contract.md`. The original plan
is preserved as planning history. This revision keeps its exact schema-v2 replay-time
design while replacing the dual-recorder implementation and verification design with
the sole supported Windows production recorder.
...
- a bounded common finalizer and atomic candidate/metadata publication;
...
There is no implicit rounding or saturating timeline conversion.
...
5. stages metadata atomically;
6. renames the candidate to canonical `video.mp4`, then publishes metadata;
7. leaves every failure noncanonical and preserves only the documented partial
   candidate.
...
midpoint. Finite advancing game clocks map through one persisted integer affine fit.
Events and snapshots use the same mapping and report before/inside/after/unavailable
rather than saturating or dropping.
...
load has a generation and every dispatch a monotonically increasing epoch. Only a
matching current-generation/current-epoch presented observation can settle authority.
Browser `currentTime` includes the validated media presentation origin; RVFC
`mediaTime` is presented-frame evidence when available.

Clip ranges are media-bound half-open frame intervals. Export reloads schema v2,
rechecks identity/facts, derives precise seek/trim values from exact rationals, trims
video and audio to one canonical interval, resets output PTS, and validates zero-based
output, exact source/output frame identity, decode, and A/V markers. Source media is
opened read-only and hash-checked before and after.
...
6. Run short exact-grid/A/V proof, native failure scenarios, 30-minute resource soak,
   ...
   verification, final review, and canonical completion updates.
...
The short marker run must emit a manifest/result binding the fixture generation,
native runner result, source/copy hashes, exact packet/frame grid, decoded marker
validity, three A/V measurements, decode/hash facts, and terminal native counters.
No synthetic source may be relabeled as native recorder evidence.

The mux lifecycle suite must cover normal close, target close, injected NVENC failure,
FFmpeg/mux process loss, and pre-first-fragment interruption. It must distinguish a
preserved partial candidate from a validated/recoverable/canonical file.

The finalizer harness must use the production finalizer code over at least five
dedicated fixtures, include a one-hour file, repeat measurements sufficiently to
calculate p95, and retain bounded JSON evidence.
