...
`docs/exec-plans/qb-replay-012-canonical-replay-time-contract-v4.md`. V4 and every
...
track metadata. Immediately before the fixed native H.264 input, the mux command uses
the exact ordered argv subsequence `-fflags nobuffer -probesize 32 -analyzeduration 0
-fpsprobesize 0 -r 60 -f h264 -i pipe:0`. The first NVENC access unit carries the
required SPS/PPS, so the packaged runtime needs no duration- or frame-rate-discovery
window; the explicit `-r 60` remains the raw-input packet clock. Focused construction
tests pin spelling, values, order, and input scope.
...
`moof`/`mdat`; mux-plan tests assert the exact bounded-analysis argv subsequence.
