# QB-REPLAY-009: Playback boundary and speed ladder (v3)

This version supersedes `docs/exec-plans/qb-replay-009-playback-boundary-and-speed-ladder-v2.md`
for the user's explicit 2026-09-09 decoder-evidence clarification. Preserve v1 and
v2 as immutable history. This plan is self-contained. The controller, rate,
audio, resource and proportionality design is retained; the decoder acceptance
prescription below replaces the invalid player-property-to-load inference.
Execution history and the forward cursor belong only to the linked checkpoint.
...
Hardware decoding remains the required normal production path for canonical
supported media, with normal WebView2 GPU acceleration enabled. Unknown is no
proof of hardware or software decoding. A proven software path is a compatibility
fallback, excluded from hardware-normal performance claims. The normal app is
not required to identify every arbitrary reload/recovery generation when
WebView2/CDP supplies no trustworthy provenance.

Retain the existing fixed internal native WebView2 diagnostic commands and event
subscriptions in `app/src-tauri/src/playback_diagnostics.rs` and
`playback_diagnostics/native.rs`, plus `playbackDiagnostics.ts`. The native monitor
associates the exact per-generation loopback URL/session token with the primary
DOM marker where a node is exposed. Bindings are monotonic; recovery rotates the
token on the same element and invalidates old evidence. CDP decoder properties
have player scope, with no load ID or property timestamp; discovery may replay an
active player and property delivery may cross load boundaries. Therefore this
source's runtime current-load result remains Unknown with absent decoder name
and platform flag, even when the player association is exact. Keep player-scoped
observations separate. Never infer ownership from arrival order or a delay.

Preserve the existing five subscriptions, one initial Browser.getVersion and
Media.enable, bounded candidate DOM descriptions, Media.disable/handler removal,
five-second acquisition deadline, eight candidates, 64 properties per candidate,
16 queued events and 64 KiB queue budget. Missing protocol, timeout, overflow,
ambiguous association and stale sessions stay explicit. Normal playback never
waits for decoder diagnostics. No new normal-runtime observer, tracing/ETW,
polling, per-frame instrumentation, remote debugging port or GPU-disable flag is
justified to make the diagnostic positive.

Establish hardware acceptance in one bounded controlled production measurement,
using the ordinary primary video/controller/adapter and canonical validated
`native-current-v2` H.264 High 1920x1080 yuv420p 60 FPS/AAC fixture, game
`1787904000`, in the existing sentinel test library. Use a fresh app/WebView process
and isolated test profile. Complete the existing Media subscription before the
first source assignment; allow exactly one primary video, controller open,
adapter source/load and CDP kLoad with its unique session URL. Exclude startup
capability-probe media in this acceptance run. Record enough lifecycle evidence
to exclude pre-existing/competing players, loads, recovery, navigation and source
changes independently of decoder property delivery order.

Reuse the existing benchmark-feature release build, sentinel runner, bounded
event writer, diagnostic-ready promise and ignored temporary probe pattern.
Where current snapshots intentionally omit raw properties, a temporary
acceptance-only tap may retain the existing raw CDP events, subscription-ready,
binding/marker and loss observations through those seams. Retention is bounded;
no synchronous file I/O in COM callbacks or hot-frame telemetry is added. Retain
the exact temporary patch/procedure and binary identity, then detach it and build
the ordinary production executable. No permanent benchmark architecture or new
schema/analyzer is needed. The tap must not choose a decoder or modify normal
decoding, recovery, rate or audio policy.

For the sole matched player require reviewed `D3D11VideoDecoder` together with
`kIsPlatformVideoDecoder=true`, real advancing RVFC presentation, and a complete
bounded transcript through the observed interval with zero recorded loss and no
contradictory decoder/error observations. Record fresh process/runtime/device,
effective launch configuration, fixture hashes, single-load/source/generation
counts and explicit attribution prerequisites. Any missing prerequisite, extra
player/load, unknown wrapper/name, event loss or software observation makes this
hardware capture inconclusive or failed. Do not relabel runtime Unknown.

This evidence is an inference from independently controlled isolation, not a CDP
load-provenance API guarantee. Its claim is hardware use for the recorded normal
canonical production path, device/runtime and observed load/interval; it does
not identify later reloads or certify all devices, codecs or future generations.
Recovery acceptance separately proves invalidation and truthful Unknown using
the existing corrective evidence. Earlier withdrawn decoder claims stay withdrawn.

Do not repeat a capture without a new concrete hypothesis addressing a material
missing prerequisite. If the bounded capture cannot establish these conditions,
keep hardware unverified and record the precise blocker; do not expand to tracing,
ETW or a diagnostic campaign. Reuse the existing export playback smoke and the
accepted unsupported HEVC disposition. Human audible/A-V observations remain
separate gates; property values, AAC presence and video frames cannot supply them.
...
- Browser capability predictions and generic GPU counters cannot prove the
  decoder selected for a player. Use bounded native diagnostics with controlled load isolation for positive acceptance.
...
Run these commands against the completed app changes when their dependency cone
changes; retain recorded passing results for unchanged code. A temporary acceptance
attachment requires a release build and focused attachment verification, then an
ordinary build after detachment. Documentation-only clarification needs canonical
validation, not replay of the completed M1-M5 checks:
...
| Expected decoder path (after) | Run the single-load controlled production capture above. Separately retain conservative runtime diagnostics and recovery invalidation evidence. | Exact canonical codec/profile, fresh process and complete single-load/player isolation, D3D11VideoDecoder plus platform flag, advancing RVFC, zero loss/contradiction and normal GPU configuration. Unknown never passes hardware acceptance. No continuous arbitrary-generation identification or decoder/vendor survey. |
...
WebView GPU acceleration. Positive hardware-normal claims require the controlled, safely attributed
decoder evidence above; software and unknown observations stay separately diagnosed.
No claim extends that isolated evidence to arbitrary reload/recovery generations.
