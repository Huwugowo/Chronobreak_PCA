...
`docs/exec-plans/qb-perf-002-low-overhead-native-windows-capture-v2.md`. That reviewed
draft and the original `docs/exec-plans/qb-perf-002-gpu-agnostic-windows-capture.md`
remain unchanged as planning history. This self-contained revision removes the
dual-backend production design and defines the sole supported Windows recorder plus the
remaining physical performance proof.
